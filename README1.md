...
| 4 | 댓글 텍스트(comment), 좋아요 수(likes), 날짜(date)를 rtx5070.csv로 저장 |
...
| 저장 파일 | rtx5070.csv (샘플: rtx5070_sample.csv) |

> 수집 대상 영상 22개의 댓글을 모두 수집한 결과 총 25,787개가 수집되었다.
...
수집된 원본 데이터(rtx5070.csv, 25,787개)에 대해 전처리 전 탐색적 분석을 수행하였다.
...
compound score 0 부근에 데이터가 집중되어 있으며, 0.05를 라벨링 기준으로 설정하였다. 1차 라벨링 결과 긍정(44.1%)과 부정(55.9%)의 분포를 확인할 수 있다. 점수가 0 근처에 몰린 것은 유튜브 댓글 특성상 중립적인 표현이 많기 때문으로 해석된다.

---

## 4. 데이터 라벨링 (auto_label.py)

수집한 댓글에는 정답 라벨이 없으므로 2단계 라벨링을 수행하였다.

### 4.1 1단계: VADER 자동 라벨링
...
| 저장 파일 | rtx5070_labeled.csv |

전체 댓글의 55.1%가 부정으로 라벨링되어 부정 댓글이 다소 우세한 것을 확인할 수 있다. 이는 RTX 5070의 출시 초기 마케팅 논란과 스펙 논쟁이 반영된 결과로 해석된다.

---

## 5. 데이터 전처리 및 분석 대상 데이터 구축 (extract_train_data.py)

### 5.1 전처리 과정

라벨링된 데이터(rtx5070_labeled.csv)에 대해 다음과 같은 전처리를 수행하였으며, 전처리 완료 후 남은 데이터를 **분석 대상 데이터**로 정의하였다.

| 단계 | 내용 | 데이터 수 |
|------|------|---------|
| 원본 라벨링 데이터 | rtx5070_labeled.csv 로드 | 25,787개 |
| 빈 댓글 제거 | dropna 처리 | - |
...
위 차트에서 확인할 수 있듯이 긍정(1)과 부정(0)이 각 2,000개로 완벽하게 균형을 이루고 있다. 분석 대상 데이터(23,048개)에서는 부정(55.1%)이 우세했으나, 균등 추출을 통해 학습 데이터의 클래스 불균형을 해소하였다. 균형 잡힌 학습 데이터는 모델이 특정 클래스에 편향되지 않고 긍정/부정을 균형 있게 학습하도록 한다.
...
MobileBERT 모델에 입력하기 위해 댓글 텍스트를 토큰화하였다. 토큰화란 텍스트를 모델이 이해할 수 있는 숫자 형태로 변환하는 과정이다.
...
Epoch 7에서 검증 정확도 **85.75%** 로 최고치를 기록하였다. Epoch 1의 Loss가 비정상적으로 높은 것은 사전학습 모델이 초기 학습 데이터에 적응하는 과정에서 발생하는 현상으로, Epoch 2부터 급격히 안정화된다. 학습 정확도와 검증 정확도의 차이가 약 13% 수준으로 일정 수준의 과적합이 발생하였으나, 검증 정확도가 Epoch 4 이후 85% 이상을 꾸준히 유지하여 학습이 안정적으로 진행되었음을 확인할 수 있다.

---

## 7. 소비자 반응 분석 결과 (analyze.py)

학습된 MobileBERT 모델로 전체 분석 대상 데이터 23,048개에 대해 감성 분류(Inference)를 수행한 결과는 다음과 같다.
...
- **데이터 수집 확대:** 수집된 25,787개의 댓글은 RTX 5070 관련 주요 영상의 댓글을 포함하고 있으나, 향후 더 다양한 채널과 영상을 추가하여 데이터 다양성을 높일 수 있다.
