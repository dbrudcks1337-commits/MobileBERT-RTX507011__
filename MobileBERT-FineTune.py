...
def main():
    ...
    # 3. 텍스트 데이터의 토큰화
    tokenizer = MobileBertTokenizer.from_pretrained('google/mobilebert-uncased')  # ← 경로 수정
    ...
    for e in range(epoch):
        ...
        for batch in process_bar:
            ...
            # Backward Pass : 손실값을 바탕으로 모델의 수를 업데이트 한다
            loss.backward()
        ...
        # --- 학습 정확도 평가 ---
        # 이 단계는 매우 무겁기 때문에 통산적으로 생략하고 loss를 많이 본다
        model.eval()
    ...
    for idx, (loss, tacc, vacc) in enumerate(epoch_results, start=1):
        print(f"Epoch {idx}: 학습오차 - {loss:.4f}, 학습정확도 - {tacc:.4f}, 검증정획도 - {vacc:.4f}")

        # 9.모델 저장
    print("n\=== 모델 저장===")
    model.save_pretrained('mobilebert_final.tp')
    print(" 모델 저장 완료")
